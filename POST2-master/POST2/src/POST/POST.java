...
public class POST {
    ...
    public void processTransaction(Transaction transaction) {
        try {
            NetClientGet.NetClientGet.postTransaction(transaction);
...
//  }
    ...
    public double getSubTotal(List<TransactionItem> items) {
        double total = 0;

        for (TransactionItem item : items) {
            total += item.getProdQuantity() * getProductPrice(item.getProductUPC());
        }
